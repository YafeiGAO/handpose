...
train_para = {'lr': [1e-5, 1e-6, 1e-7],
              ...
              'show_loss_freq': 1000,
              'snapshot_freq': 5000,
              'snapshot_dir': 'snapshots_handsegnet_variational_1'}
...
hand_mask_pred,loss_z = net.inference_detection(data['image'], train=True)
...
for i, pred_item in enumerate(hand_mask_pred):
    ...
    loss += tf.reduce_mean(tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(logits=pred, labels=gt))+loss_z)
    #print(loss)
...
rename_dict = {'CPM/PersonNet': 'HandSegNet',
               '_CPM': ''}
#load_weights_from_snapshot(sess, './weights/cpm-model-mpii', ['PoseNet', 'Mconv', 'conv6'], rename_dict) #yafei@@
